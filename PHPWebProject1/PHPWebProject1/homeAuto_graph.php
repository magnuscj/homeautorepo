...
if(isCli())
{
    ...
    $sleepTime = $SLEEP_5MIN ;//getConfig("SLEEP")+20;
}
else
{
    $path = "pictures\\".$fileName;
...
do
{
	...
	foreach($sensors[$colID] as $sensorId)
	{
		...
		if($sensorShow[$senNo] == "on" && $sensors[$colType][$senNo] == "power")
		{
			...
			$lineplot2->SetColor($sensors[$colColor][$senNo]);
			$lineplot2->SetFillGradient('darkred','orange@0.4');
	...
	if(isCli())
	{
		...
		$utr = time()-$time;
		print date('H:i:s',time()).", finished, it took "."$utr"." seconds. Now sleep for "."$sleepTime"."seconds. \n\n";
		sleep($sleepTime);
	}
	else
	{
